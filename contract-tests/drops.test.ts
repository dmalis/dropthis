...
describe("text is served as UTF-8", () => {
  ...
  it("declares charset on the drop root, the file path and the download route", async () => {
    ...
    // The viewer is public; the API's file route needs the key. Both declare
    // the charset, which is the claim under test.
    const fetches: Array<[url: string, get: () => Promise<Response>]> = [];
    for (const url of [
      drop.url as string,
      `${drop.url as string}index.html`,
      `${drop.url as string}notes.md`,
    ]) {
      fetches.push([url, () => fetch(url, { cache: "no-store" })]);
    }
    for (const path of ["notes.md", "index.html"]) {
      const url = `${BASE_URL}/_api/v1/drops/${slug}/files/${path}`;
      fetches.push([url, () => api(`/_api/v1/drops/${slug}/files/${path}`)]);
    }

    for (const [url, get] of fetches) {
