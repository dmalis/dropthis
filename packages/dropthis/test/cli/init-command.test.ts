...
/**
 * A wrangler that behaves like a deploy: it registers the script with the fake
 * account API and starts the real Worker app over the bucket the installer
 * wrote, then writes that URL where the test can read it.
 */
async function stubWranglerBinary(cfOrigin: string): Promise<string> {
