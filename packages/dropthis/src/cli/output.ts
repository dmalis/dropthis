 ...
 *
 * Which operation gets a one-line plain rendering, and whether that line is
 * the result (stdout, so it pipes) or a note about it (stderr, leaving stdout
 * empty), is declared on the operation entry — never matched by name here
 * (#28).
 */
export function renderResult(io: Io, mode: Mode, op: Operation<never>, value: unknown): void {
  if (mode !== "plain") {
    io.stdout.write(jsonLine(value));
    return;
  }
  const plain = op.cli?.plain;
  if (plain !== undefined) {
    (plain.stream === "stdout" ? io.stdout : io.stderr).write(`${plain.line(value)}\n`);
    return;
  }
